...
#else
  TORCH_WARN_ONCE(
      "onednn_grouped_gemm_w4a8: Consider using "
      "onednn_grouped_gemm_w4a8_prepacked for better performance. "
      "Prepack weights once at load time with "
...
std::tuple<torch::Tensor, torch::Tensor> grouped_gemm_w4a8_prepack(
    torch::Tensor B_packed_u4,
    torch::Tensor B_scales) {
  auto B_s4 = (B_packed_u4 ^ 0x88).contiguous();
  auto B_scales_permuted = B_scales.permute({0, 2, 1}).contiguous();
  return std::make_tuple(std::move(B_s4), std::move(B_scales_permuted));
}
...
torch::Tensor grouped_gemm_w4a8_prepacked(
  ...
  CHECK_DEVICE(A_zp);
  CHECK_DEVICE(B_s4);
  CHECK_DEVICE(B_scales_permuted);
  CHECK_DEVICE(D);
  CHECK_DEVICE(expert_first_token_offset);
  ...
  CHECK_CONTIGUOUS(A_scale);
  CHECK_CONTIGUOUS(A_zp);
  CHECK_CONTIGUOUS(B_s4);
  CHECK_CONTIGUOUS(B_scales_permuted);
  CHECK_CONTIGUOUS(D);
  CHECK_CONTIGUOUS(expert_first_token_offset);

  TORCH_CHECK(A_q.dim() == 2, "A_q must be 2D [Total_M, K]");
  TORCH_CHECK(B_s4.dim() == 3, "B_s4 must be 3D [E, N, K/2]");
  TORCH_CHECK(
      B_scales_permuted.dim() == 3,
      "B_scales_permuted must be 3D [E, G, N]");
  TORCH_CHECK(D.dim() == 2, "D must be 2D [Total_M, N]");
  TORCH_CHECK(
      expert_first_token_offset.dim() == 1,
      "expert_first_token_offset must be 1D [E+1]");
  ...
  TORCH_CHECK(
      ...
      "A_zp must be uint8, int32, or int64 for oneDNN grouped_gemm_w4a8");
  TORCH_CHECK(
      B_s4.scalar_type() == at::ScalarType::Byte,
      "B_s4 must be uint8 (prepacked s4) for oneDNN grouped_gemm_w4a8");
  TORCH_CHECK(
      is_fp16_bf16_or_fp32(B_scales_permuted.scalar_type()),
      "B_scales_permuted must be fp16, bf16, or fp32");
  TORCH_CHECK(
      is_fp16_or_bf16(D.scalar_type()),
      "D must be fp16 or bf16 for oneDNN grouped_gemm_w4a8");
  ...
  TORCH_CHECK(
      B_s4.size(0) == num_experts, "B_s4.size(0) must match num_experts");
  TORCH_CHECK(B_s4.size(1) == N, "B_s4.size(1) must match N");
  TORCH_CHECK(
      B_s4.size(2) * 2 == K,
      "B_s4.size(2) must be K/2 bytes for packed int4 weights");

  TORCH_CHECK(
      B_scales_permuted.size(0) == num_experts,
      "B_scales_permuted.size(0) must match num_experts");
  TORCH_CHECK(
      B_scales_permuted.size(2) == N,
      "B_scales_permuted.size(2) must match N");
  TORCH_CHECK(
      B_scales_permuted.size(1) > 0,
      "B_scales_permuted.size(1) (group_num) must be > 0");
  TORCH_CHECK(
      K % B_scales_permuted.size(1) == 0,
      "B_scales_permuted.size(1) (group_num) must divide K");
  ...
  auto run_int8 = [&](dnnl::memory::data_type dst_dt) {
    ...
    try {
      ...
      if (iter == primitive_cache.end()) {
        ...
        try {
          ...
          if (e.status == dnnl_unimplemented ||
            ...
            TORCH_CHECK(
                false,
                "oneDNN grouped_gemm_w4a8_prepacked: matmul primitive_desc "
                "creation failed on GPU. Detail: ",
                e.what());
          }
          throw;
        }
      ...
      if (e.status == dnnl_unimplemented ||
        ...
        TORCH_CHECK(
            false,
            "oneDNN grouped_gemm_w4a8_prepacked: GPU execution failed. "
            "Detail: ",
            e.what());
      }
      if (e.status == dnnl_unimplemented) {
        TORCH_CHECK(
            false,
            "oneDNN grouped_gemm_w4a8_prepacked: native int8xint4 grouped "
            "matmul is unimplemented. "
            "scale dtypes: A_scale=",
            torch_dtype_name(A_scale.scalar_type()),
            " (oneDNN=",
            dnnl_dt2str(static_cast<dnnl_data_type_t>(src_scales_dt)),
            "), B_scales=",
            torch_dtype_name(B_scales_permuted.scalar_type()),
            " (oneDNN=",
            dnnl_dt2str(static_cast<dnnl_data_type_t>(wei_scales_dt)),
            "); zero-point dtypes: A_zp=",
            dnnl_dt2str(static_cast<dnnl_data_type_t>(src_zp_dt)),
      ...
      TORCH_WARN(
          "oneDNN grouped_gemm_w4a8_prepacked: int8 primitive "
          "creation/execute failed (dst_dt=",
          static_cast<int>(dst_dt),
          "): status=",
          static_cast<int>(e.status),
          " (",
      ...
      if (message_contains(e.what(), "unsupported scales configuration")) {
        TORCH_CHECK(
            false,
            "oneDNN grouped_gemm_w4a8_prepacked: GPU execution failed with an "
            "unsupported scales configuration. Detail: ",
            e.what());
      }
      throw;
    }
